...
test.group('Driver vehicle', () => {+
    test('List one Driver-vehicle', async ({ client }) => {
      ...
      const response = await client.get('/driver-vehicles/1').loginAs(admin)
    ...
    test('Create a Driver-vehicle', async ({ client, assert }) => {
      ...
      const response = await client.post('/driver-vehicles').json({
    ...
    test('Delete a Driver-vehicle', async ({ client, assert }) => {
      ...
      const destroy_response = await client.delete(`/driver-vehicles/${new_driverVehicle.id}`).loginAs(admin)
